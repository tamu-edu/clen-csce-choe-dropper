...
# 
# inp=(0:0.1:1.0)'; outp=[sin(inp*(2*pi))]/2; plot(outp); p.n_hidden=12; p.n_individuals=60; p.retention_rate=30/p.n_individuals; p.mutation_rate=4/p.n_hidden; p.max_generation=1000; p.mutation_magnitude=1.5; p.error_thresh=0.0005; [gp,err,midx] = ga(inp,[outp], p); ih=gp(midx).input_to_hidden; ho=gp(midx).hidden_to_output; inpb=[inp,-ones(rows(inp),1)]; hact=tanh(inpb*ih); [r,c]=size(hact); hact(:,c)=-ones(r,1); gset yrange [-1:1]; t=tanh(hact*ho); plot(inp,outp(:,1),inp,t(:,1),"3*-");
...
for i=1:n_individuals
   ...
   fprintf(stdout,"[Err=%f, f=%d, 1=%d, 2=%d]",err(i),n(1),n(2),n(3));
   fflush(stdout);
