...
#
# function [gene_pool,gen_err,min_net,best_net] = dynrga(params[,saved_gene_pool,individual])
#
...
#	params.output_noise  : add noise in output
#
#	Dropper-specific parameters
#
#	params.no_recurrency : 1 = feedforward 0 = recurrent 
#	params.num_drops     : number of ball drops per individual
#	params.arena_size    : size of bitmap rendering of the ball-dropping
#			       arena = n, for (n x n)
#	params.num_rays      : number of sweeping rays
...
function [gene_pool,gen_err,min_net,best_net] = dynrga(params,saved_gene_pool,saved_individual)
...
if (params.no_recurrency)
    n_input = params.num_rays+1;	# "+1" for bias unit
else 
...
if (test_mode==1)
  gene_pool = saved_gene_pool;
  params.max_generation=5;
else 
...
# set up ball initial position and velocity for each generation, for
# a fair comparison 
...
# obj_list
# abs(obj_list(:,1)-obj_list(:,2))
vel_list = zeros(params.num_drops,2);
vel_list((1:2:params.num_drops),:)=ones(length(1:2:params.num_drops),1)*[1,2];
vel_list((2:2:params.num_drops),:)=ones(length(2:2:params.num_drops),1)*[2,1];
vel_list;
...
for i=1:n_individuals
   ...
   load -force output_noise.dat;

   for k=1:params.num_drops

	if (test_mode ==1)
		if (k!=saved_individual)
			continue;
		end
	end
	...
	if (test_mode == 1)
	objs = obj_list(ceil(rand*params.num_drops),:);
	vels = vel_list(ceil(rand*params.num_drops),:);
 	else
	objs = obj_list(k,:);
	vels = vel_list(k,:);
	end
	...
	while (1) 
		...
		prev_hidden=0.3*hidden(1:params.n_hidden)+0.7*prev_hidden;
		output = tanh(hidden*gene_pool(i).hidden_to_output);

		#--------------------
		# move decision
		#--------------------
		if ((output(1)+(output(1)+output(2))/2*(rand-0.5)*output_noise)>output(2))
   ...
   if (gen_err(g)<best_err)
	best_err = gen_err(g);
	best_gp = gene_pool(min_net);
	best_gen = g;
   end	
...
#
figure(2);
#yc eval(sprintf("gset_stub(\"xrange [*:*]\")",params.error_thresh));
#yc eval(sprintf("gset_stub(\"yrange [%f:*]\")",params.error_thresh));
loglog(err,"1-",err,"3*");
